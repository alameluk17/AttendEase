...
def main(context):
    ...
    try:
        ...
        if "courseId" not in context.req.body_json:
            raise ValueError("`courseId` must be provided in the JSON Request Body.")
        ...
        print(moodleUserId)
        courseDetails = moodle.core_enrol_get_enrolled_users(courseid=context.req.body_json["courseId"])
